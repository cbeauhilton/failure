...
combo.to_csv(config.TABLES_DIR / "genes_misclassified.csv")
...
force_plot_csv.to_csv(config.TABLES_DIR / "genes_pt_nums.csv")
...
csv_file = config.TABLES_DIR / "genes_shap_df.csv"
h5_file = config.PROCESSED_DATA_DIR / "everything.h5"
imp_cols.to_csv(csv_file)
imp_cols.to_hdf(h5_file, key="genes_ordered_shap_cols", format="table")
...
plt.savefig(
    config.FIGURES_DIR / "shap_images/shap_summary"/ f"genes_shap_summary_bar_all_classes.pdf", bbox_inches="tight"
)
plt.close()
...
for i, classname in enumerate(CLASSES):
    try:
        ...
        plt.title(f"{classname.upper()}")
        plt.savefig(
            config.FIGURES_DIR / f"shap_images/shap_summary/genes_shap_summary_dot_{classname}_{i}.pdf",
            bbox_inches="tight",
        )
        plt.close()
        ...
        plt.title(f"{classname.upper()}")
        plt.savefig(
            config.FIGURES_DIR / f"shap_images/shap_summary/genes_shap_summary_bar_{classname}_{i}.pdf",
            bbox_inches="tight",
        )
        plt.close()
    ...
    try:
        ...
        plt.title(f"{classname.upper()}")
        plt.savefig(
            config.FIGURES_DIR / f"shap_images/shap_summary/genes_shap_summary_compact_dot_{classname}_{i}.pdf",
            bbox_inches="tight",
        )
        plt.close()
    ...
    for col in list(X):
        try:
            ...
            plt.savefig(config.FIGURES_DIR / f"shap_images/shap_dependence/{classname}/genes_shap_dependence_{classname}_{col}.pdf", transparent=True)
            plt.close()
        except Exception as ex:
            exhandler(ex, module=os.path.basename(__file__))
    ...
    for pt_num in pt_nums:
        try:
            ...
            plt.savefig(
                config.FIGURES_DIR
                / f"shap_images/force_plots/genes_forceplot_{classname}_{i}_pt_{pt_num}.pdf",
                bbox_inches="tight",
                transparent=True,
            )
            plt.close()
...
imp_cols = pd.read_csv(config.TABLES_DIR / "genes_shap_df.csv")
prettycols = pd.read_csv(config.TABLES_DIR / "prettify.csv")
di = dict(zip(prettycols.ugly, prettycols.pretty_full))
print(di)
...
pretty_imp_cols.to_csv(config.TABLES_DIR / "genes_shap_df_pretty.csv")
